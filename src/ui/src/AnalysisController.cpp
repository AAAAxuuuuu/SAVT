...
QVariantMap buildCapabilityNodeDetailsMap(const core::CapabilityNode &node) {
  QVariantMap item;
  item.insert(QStringLiteral("id"), static_cast<qulonglong>(node.id));
  item.insert(QStringLiteral("kind"),
              QString::fromStdString(core::toString(node.kind)));
  ...
  item.insert(QStringLiteral("responsibility"),
              QString::fromStdString(node.responsibility));
  item.insert(QStringLiteral("summary"), QString::fromStdString(node.summary));
  item.insert(QStringLiteral("folderHint"),
              QString::fromStdString(node.folderHint));
  item.insert(QStringLiteral("moduleNames"),
              toVariantStringList(node.moduleNames));
  item.insert(QStringLiteral("exampleFiles"),
              toVariantStringList(node.exampleFiles));
  item.insert(QStringLiteral("topSymbols"),
              toVariantStringList(node.topSymbols));
  item.insert(QStringLiteral("collaboratorNames"),
              toVariantStringList(node.collaboratorNames));
  item.insert(QStringLiteral("supportingRoles"),
              toVariantStringList(node.supportingRoles));
  item.insert(QStringLiteral("technologyTags"),
              toVariantStringList(node.technologyTags));
  item.insert(QStringLiteral("riskSignals"),
              toVariantStringList(node.riskSignals));
  item.insert(QStringLiteral("moduleCount"),
              static_cast<qulonglong>(node.moduleCount));
  item.insert(QStringLiteral("fileCount"),
              static_cast<qulonglong>(node.fileCount));
  item.insert(QStringLiteral("sourceFileCount"),
              static_cast<qulonglong>(node.sourceFileCount));
  item.insert(QStringLiteral("qmlFileCount"),
              static_cast<qulonglong>(node.qmlFileCount));
  item.insert(QStringLiteral("webFileCount"),
              static_cast<qulonglong>(node.webFileCount));
  item.insert(QStringLiteral("scriptFileCount"),
              static_cast<qulonglong>(node.scriptFileCount));
  item.insert(QStringLiteral("dataFileCount"),
              static_cast<qulonglong>(node.dataFileCount));
  item.insert(QStringLiteral("stageStart"),
              static_cast<qulonglong>(node.stageStart));
  item.insert(QStringLiteral("stageEnd"),
              static_cast<qulonglong>(node.stageEnd));
  item.insert(QStringLiteral("incomingEdgeCount"),
              static_cast<qulonglong>(node.incomingEdgeCount));
  item.insert(QStringLiteral("outgoingEdgeCount"),
              static_cast<qulonglong>(node.outgoingEdgeCount));
  item.insert(QStringLiteral("aggregatedDependencyWeight"),
              static_cast<qulonglong>(node.aggregatedDependencyWeight));
  item.insert(QStringLiteral("flowParticipationCount"),
              static_cast<qulonglong>(node.flowParticipationCount));
  item.insert(QStringLiteral("collaboratorCount"),
              static_cast<qulonglong>(node.collaboratorCount));
  item.insert(QStringLiteral("riskScore"),
              static_cast<qulonglong>(node.riskScore));
  item.insert(QStringLiteral("riskLevel"),
              QString::fromStdString(node.riskLevel));
  item.insert(QStringLiteral("priority"),
              static_cast<qulonglong>(node.visualPriority));
  item.insert(QStringLiteral("defaultVisible"), node.defaultVisible);
  ...
  item.insert(QStringLiteral("laneGroupId"),
              static_cast<qulonglong>(node.laneGroupId));
  item.insert(QStringLiteral("detailGroupId"),
              static_cast<qulonglong>(node.detailGroupId));
  return item;
}

bool hasCapabilityEvidenceDetails(const QVariantMap &nodeData) {
  return nodeData.contains(QStringLiteral("exampleFiles")) &&
         nodeData.contains(QStringLiteral("topSymbols")) &&
         nodeData.contains(QStringLiteral("collaboratorNames")) &&
         nodeData.contains(QStringLiteral("moduleNames"));
}
...
void populatePendingAnalysisResult(
  ...
  result.capabilityGraph =
      std::make_shared<core::CapabilityGraph>(capabilityGraph);
...
void AnalysisController::copyCodeContextToClipboard(const qulonglong nodeId) {
  const QVariantMap targetNode = capabilityNodeDetails(nodeId);
...
void AnalysisController::requestAiExplanation(const QVariantMap &nodeData,
  ...
  const QVariantMap resolvedNodeData = mergeCapabilityNodeData(nodeData);
  ...
  AiService::logRequest(
      prepared,
      QStringLiteral("%1 (%2)")
          .arg(resolvedNodeData.value(QStringLiteral("id")).toULongLong())
...
void AnalysisController::requestProjectAiExplanation(const QString &userTask) {
  const AiPreparedRequest prepared = AiService::prepareProjectRequest(
      AiRequestContext{m_projectRootPath, m_analysisReport, m_statusMessage,
                       m_analysisPhase, userTask},
      m_systemContextData, m_systemContextCards, m_capabilityGraph.get());
  applyAiAvailability(prepared.availability);
  if (!prepared.ready) {
    setAiStatusMessage(prepared.failureStatusMessage);
    return;
...
void AnalysisController::finishAnalysis() {
    ...
    setCapabilityGraph(result->capabilityGraph);
...
void AnalysisController::setCapabilityGraph(
    std::shared_ptr<const core::CapabilityGraph> value) {
  m_capabilityGraph = std::move(value);
  m_capabilityNodeIndex.clear();
  if (!m_capabilityGraph)
    return;

  m_capabilityNodeIndex.reserve(m_capabilityGraph->nodes.size());
  for (std::size_t index = 0; index < m_capabilityGraph->nodes.size();
       ++index) {
    m_capabilityNodeIndex.emplace(m_capabilityGraph->nodes[index].id, index);
  }
}
...
const core::CapabilityNode *
AnalysisController::capabilityNodeById(const std::size_t nodeId) const {
  if (!m_capabilityGraph)
    return nullptr;

  if (const auto it = m_capabilityNodeIndex.find(nodeId);
      it != m_capabilityNodeIndex.end() &&
      it->second < m_capabilityGraph->nodes.size()) {
    return &m_capabilityGraph->nodes[it->second];
  }

  for (const core::CapabilityNode &node : m_capabilityGraph->nodes) {
    if (node.id == nodeId)
      return &node;
  }
  return nullptr;
...
QVariantMap AnalysisController::capabilityNodeDetails(
    const qulonglong nodeId) const {
  if (const core::CapabilityNode *node =
          capabilityNodeById(static_cast<std::size_t>(nodeId));
      node != nullptr) {
    return buildCapabilityNodeDetailsMap(*node);
  }
  return {};
}

QVariantMap AnalysisController::mergeCapabilityNodeData(
    QVariantMap nodeData) const {
  if (nodeData.isEmpty() || hasCapabilityEvidenceDetails(nodeData))
    return nodeData;

  const qulonglong nodeId = nodeData.value(QStringLiteral("id")).toULongLong();
  if (nodeId == 0)
    return nodeData;

  const QVariantMap detailData = capabilityNodeDetails(nodeId);
  for (auto it = detailData.constBegin(); it != detailData.constEnd(); ++it) {
    if (!nodeData.contains(it.key()))
      nodeData.insert(it.key(), it.value());
  }
  return nodeData;
}
