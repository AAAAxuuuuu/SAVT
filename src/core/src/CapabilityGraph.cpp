...
std::vector<std::string> splitPathSegments(const std::string& path) {
    ...
    for (const char character : path) {
        if (character == '/' || character == '\\') {
            if (!current.empty()) {
                segments.push_back(lowerCopy(current));
                current.clear();
            }
            continue;
        }
...
bool isExampleLikeSegment(const std::string& segment) {
    static const std::unordered_set<std::string> segments = {
        "example", "examples", "sample", "samples", "demo", "demos",
        "playground", "sandbox", "prototype", "prototypes",
        "bench", "benchmarks"
    };
    return segments.contains(segment);
}

bool isGeneratedLikeSegment(const std::string& segment) {
    static const std::unordered_set<std::string> segments = {
        "generated", "generatedfiles", "autogen",
        "autogenerated", ".generated", "gen"
    };
    return segments.contains(segment);
}

bool isTestSupportSegment(const std::string& segment) {
    static const std::unordered_set<std::string> segments = {
        "test", "tests", "testdata", "fixture", "fixtures",
        "mock", "mocks", "stub", "stubs", "__tests__"
    };
    return segments.contains(segment);
}

AuxiliaryNoiseKind classifyAuxiliaryNoisePath(const std::string& path) {
    const std::vector<std::string> segments = splitPathSegments(path);
    if (segments.empty()) {
        return AuxiliaryNoiseKind::None;
    }
    ...
    if (std::any_of(
            ...
            [](const std::string& segment) {
                return isGeneratedLikeSegment(segment);
            })) {
        return AuxiliaryNoiseKind::GeneratedLike;
    }

    const std::string& firstSegment = segments.front();
    if (isExampleLikeSegment(firstSegment)) {
        return AuxiliaryNoiseKind::ExampleLike;
    }
    if (isTestSupportSegment(firstSegment)) {
        return AuxiliaryNoiseKind::TestSupport;
    }
    return AuxiliaryNoiseKind::None;
}

AuxiliaryNoiseKind classifyAuxiliaryNoiseNode(const CapabilityNode& node) {
    if (node.kind == CapabilityNodeKind::Entry ||
        node.dominantRole == "dependency") {
        return AuxiliaryNoiseKind::None;
    }

    AuxiliaryNoiseKind classification = AuxiliaryNoiseKind::None;
    bool sawCandidatePath = false;
    auto consumePath = [&](const std::string& path) {
        if (path.empty()) {
            return true;
        }

        const AuxiliaryNoiseKind pathKind = classifyAuxiliaryNoisePath(path);
        if (pathKind == AuxiliaryNoiseKind::None) {
            return false;
        }

        sawCandidatePath = true;
        if (classification == AuxiliaryNoiseKind::None) {
            classification = pathKind;
            return true;
        }
        return classification == pathKind;
    };

    for (const std::string& moduleName : node.moduleNames) {
        if (!consumePath(moduleName)) {
            return AuxiliaryNoiseKind::None;
        }
    }
    for (const std::string& filePath : node.exampleFiles) {
        if (!consumePath(filePath)) {
            return AuxiliaryNoiseKind::None;
        }
    }
    if (!node.folderHint.empty() && !consumePath(node.folderHint)) {
        return AuxiliaryNoiseKind::None;
    }

    return sawCandidatePath ? classification : AuxiliaryNoiseKind::None;
}

std::string auxiliaryNoiseSummary(
    const AuxiliaryNoiseKind kind,
    const std::size_t fileCount) {
    switch (kind) {
    case AuxiliaryNoiseKind::ExampleLike:
        return "Example or demo workspace (" + std::to_string(fileCount) +
               " file(s)), folded by default to keep the primary architecture view focused.";
    case AuxiliaryNoiseKind::GeneratedLike:
        return "Generated source area (" + std::to_string(fileCount) +
               " file(s)), folded by default to keep the primary architecture view focused.";
    case AuxiliaryNoiseKind::TestSupport:
        return "Test or fixture support area (" + std::to_string(fileCount) +
               " file(s)), folded by default to keep the primary architecture view focused.";
    case AuxiliaryNoiseKind::None:
        break;
    }
...
std::size_t applyAuxiliaryNoiseCollapse(CapabilityGraph& graph) {
    std::size_t focusNodeCount = 0;
    for (const CapabilityNode& node : graph.nodes) {
        ...
        if (classifyAuxiliaryNoiseNode(node) == AuxiliaryNoiseKind::None) {
            ++focusNodeCount;
        }
    }
    ...
    for (CapabilityNode& node : graph.nodes) {
        const AuxiliaryNoiseKind kind = classifyAuxiliaryNoiseNode(node);
        if (kind == AuxiliaryNoiseKind::None) {
            continue;
        }
        ...
        if (node.responsibility.empty() || node.summary.empty()) {
            const std::string summary = auxiliaryNoiseSummary(kind, node.fileCount);
...
CapabilityGraph buildCapabilityGraph(const AnalysisReport& report, const ArchitectureOverview& overview) {
    ...
    const std::size_t auxiliaryFoldCount = applyAuxiliaryNoiseCollapse(graph);
    if (auxiliaryFoldCount > 0) {
        graph.diagnostics.push_back(
            "Default view folded " + std::to_string(auxiliaryFoldCount) +
            " example/generated/test-support node(s) to keep the L1/L2 view focused.");
    }
